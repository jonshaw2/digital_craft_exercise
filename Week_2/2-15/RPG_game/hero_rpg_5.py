...
class Character(object):
    ...
    def check_inventory(self):
        while True:
            ...
            for i in xrange(len(self.inventory[0])):
                item = self.inventory[0][i]
                print "%d. %s [%d]" % (i+1, self.inventory[0][i], self.inventory[1][i])
            print "10. leave"
            input = int(raw_input("> "))
            #item.apply(hero)
            ...
            if input == 10:
                ...
                if self.inventory[1][input-1] == 0:
                    print "You have no more %s" %(self.inventory[0][input-1])
                else:
                    #ItemToUse = getattr(self.inventory[0][input - 1], "apply")
                    ItemToUse = getattr(sys.modules[__name__],(self.inventory[0][input - 1]))
                    ItemToUse().apply(hero)
                    self.inventory[1][input - 1] -= 1
...
class Dopple_Ganger(Hero):
    def __init__(self):
        ...
        self.critical = hero.critical
        self.regen_chance = hero.regen_chance
        self.regen_power = hero.regen_power
        self.dodge = hero.dodge
        self.armor = hero.armor
        self.agility = hero.agility
        self.bounty = hero.coins
